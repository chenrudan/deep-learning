...
void TrainClassification<Dtype>::createDataMPIDist(int multi) {
	...
	if(this->_model_component->_pid == this->_model_component->_master_pid)
	{
		...
		num_trans = this->_model_component->_num_process - 1;
		send_pixel[0] = new MPIDistribute<Dtype>( \
				pixel_len, this->_model_component->_pid-1+num_trans*multi, 0, MPI_FLOAT, \
				this->_model_component->_mini_data[multi%2]->getDevData());	

		send_label[0] = new MPIDistribute<int>( \
				label_len, this->_model_component->_pid-1+num_trans*(multi+1), 0, MPI_INT, \
				this->_model_component->_mini_label[multi%2]->getDevData());

		this->_model_component->_send_recv_label.push_back(send_label[0]);
		this->_model_component->_send_recv_pixel.push_back(send_pixel[0]);
	}
...
void TrainClassification<Dtype>::sendAndRecvForManager() {
	int num_pars_type = 0;
	if(!this->_is_test)
		num_pars_type = this->_model_component->_num_need_train_layers;
...
#pragma omp parallel num_threads(num_threads)
	{
		...
		if(tid < num_trans*this->_num_data_type){
			int pid = tid % num_trans + 1 + this->_model_component->_master_pid;   //计算出对应的进程ID
			int type_id = tid / num_trans;   //计算是train还是valid
			do{
				if(this->_is_test){
					this->_load_layer->loadTestOneBatch( \
							this->_model_component->_send_recv_pixel[tid]->getFlag()+1, \
							pid-1-this->_model_component->_master_pid, \
