...
template <typename Dtype>
void Data<Dtype>::copyFromDevice(Data<Dtype>* data_in){
...
//	cout << sizeof(Dtype) << ":" << data_len << endl;
...
template <typename Dtype>
void Data<Dtype>::copyToDevice(Data<Dtype>* data_in){
	cudaError_t status = cudaMemcpy(data_in->getDevData(), _data_value, \
			sizeof(Dtype) * _amount, cudaMemcpyDeviceToDevice);
	if (status != cudaSuccess) {
		cout << stderr, "!!!! device access error (write)\n";
		exit( EXIT_FAILURE );
