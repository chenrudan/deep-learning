...
//		pthread_mutex_lock(&mutexS);
		if(myMsg->isSender)
			MPI_Recv(&myMsg->myState, 1, MPI_INT, myMsg->recvPid, \
					myMsg->mySwap + (numCompute+1) * (numProcess - 1), \
					MPI_COMM_WORLD, &status);
		else
			MPI_Recv(&myMsg->myState, 1, MPI_INT, myMsg->sendPid, \
					myMsg->mySwap + 1, MPI_COMM_WORLD, &status);
	cout <<rank << ":recv:" << myMsg->mySwap + (numCompute+1) * (numProcess - 1) \
				<< endl;
		if(myMsg->isSender){
			MPI_Bsend(myData, myMsg->transLen, MPI_FLOAT, myMsg->recvPid, \
					myMsg->mySwap + (numCompute+1) * (numProcess - 1), \
					MPI_COMM_WORLD);
		}
		else
...
cout << rank << ":send:" << myMsg->mySwap + myMsg->myState * (numProcess - 1) \
			<< endl;
...
void managerNode(pars* logistic){
	...
	char* myBuffer = new char[logistic->trainNum * ( 1+inSqrt ) * sizeof(float) * 2];
	int detachOut;
	int detachS;
	MPI_Buffer_attach(myBuffer, logistic->trainNum * ( 1+inSqrt) * sizeof(float) * 2);
	...
	* logistic->filterSize;
	...
	for(int epochIdx = 0; epochIdx < logistic->numEpoches; epochIdx++){
		...
		for(int batchIdx = 0; batchIdx < logistic->numMinibatches; batchIdx++){
			...
			MPI_Send(&passMsg[1], 1, MPI_INT, 0, \
					SWAP_LABEL_TRAIN + batchIdx * (numProcess - 1), \
					MPI_COMM_WORLD);
cout << "waiting receive label:"<<  SWAP_LABEL_TRAIN + batchIdx * (numProcess - 1)<< "\n";
			...
			MPI_Send(&passMsg[0], 1, MPI_INT, 0, \
					SWAP_PIXEL_TRAIN + batchIdx * (numProcess - 1), \
					MPI_COMM_WORLD);
cout << "waiting receive pixel:"<<  SWAP_PIXEL_TRAIN + batchIdx * (numProcess - 1)<< "\n";
