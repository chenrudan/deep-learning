...
public:	
	...
	Matrix(Dtype* devData, int numRows, int numCols);
	~Matrix();
	...
	inline bool isSameDims(const Matrix<Dtype>* m) const { 
		return m->getNumRows() == _shape[0] && m->getNumCols() == _shape[1];
	}
	...
	/// \param[in] c 点加
	void addSum(Matrix<Dtype>* b, Matrix* c, float scale_This, \
