		...
		for(int batchIdx = 0; batchIdx < numMinibatches; batchIdx++){
			...
			if(rank == 0){
				for(int i = 1; i < numProcess; i++){
					MPI_Send(nvTrainData->getDevData() + (i-1)*miniDataLen, \
							miniDataLen, MPI_FLOAT, i, 0, MPI_COMM_WORLD);
					MPI_Send(nvTrainLabel->getDevData() + (i-1)*miniLabelLen, \
							miniLabelLen, MPI_FLOAT, i, 0, MPI_COMM_WORLD);
				}
			}
			else{
			...
			if((batchIdx + 1) % nPush == 0){
				if(rank != 0){
					...
					for(int sender = 1; sender < numProcess; sender++){
						MPI_Recv((sender-1)*avgOutLen/(numProcess-1) \
								+ avgOut->getDevData(), avgOutLen/(numProcess-1), \
								MPI_FLOAT, sender, 0, MPI_COMM_WORLD, \
								&status);
						MPI_Recv((sender-1)*outBiasLen/(numProcess-1) \
								+ outBiases->getDevData(), outBiasLen/(numProcess-1), \
								MPI_FLOAT, sender, 0, MPI_COMM_WORLD, \
								&status);
					}
			...
			if(batchIdx == numMinibatches - 1){
				...
				for(int validIdx = 0; validIdx < numValidBatches; validIdx++){
					if(rank == 0){
						for(int i = 1; i < numProcess; i++){
							MPI_Send(nvValidData->getDevData() + (i-1)*miniDataLen, \
									miniDataLen, MPI_FLOAT, i, 0, MPI_COMM_WORLD);
							MPI_Send(nvValidLabel->getDevData() + (i-1)*miniLabelLen, \
									miniLabelLen, MPI_FLOAT, i, 0, MPI_COMM_WORLD);
						}
					}
					else{
						MPI_Recv(nvValidData->getDevData(), miniDataLen, MPI_FLOAT, \
								0, 0, MPI_COMM_WORLD, &status);
						MPI_Recv(nvValidLabel->getDevData(), miniLabelLen, MPI_FLOAT, \
								0, 0, MPI_COMM_WORLD, &status);
					}
					if(rank != 0){
