...
__global__ void filterKernel(cublasHandle_t handle, cublasStatus_t stat, \
	float *d_w, float *d_x, float *d_h, float al, float bet){			
	int w_start = threadIdx.x * m * k;
	int h_start = threadIdx.x * m * n;
    stat = cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n, k, \
            &al, d_w + w_start, m, d_x, k, &bet, d_h + h_start, m);
}
...
int main(){
    ...
    filterKernel<<<1, 2>>>(handle, stat, d_w, d_x, d_h, al, bet);
