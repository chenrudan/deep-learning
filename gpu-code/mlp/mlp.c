...
//#define test_length 16280
#define test_length 0
#define train_length 120
#define validation_length 45
	...
	while(epoch<n_epoches&&!done_looping){
		...
		for(minibatch_index=0;minibatch_index<n_train_batches;minibatch_index++){
			...
			if((iter+1)%validation_frequency==0){
				...
				fprintf(out, "epoch %d ,validation error %lf %% \n",epoch-1,this_validation_loss*100);
...
double logRegressionLayer(int i,int index){
	...
	//printf("index:%d       negative_log:%lf\n",index,negative_log);
	if(index==0){
		return negative_log;
	}
	else{
...
void loaddata(){
    ...
    char *address[]={"adult_rtest_label.bin","yale_train_test_label.bin","adult_rtest.bin","yale_train_test.bin"};
    ...
    if(fp!=NULL&&fread(buf,1,SIZE2/LENGTH,fp) >=0){
            ...
            if(length==0){
                for(i=0;i<test_length;i++){
                    test_label[i]=(int)buf[i];
                }
            }
            else{
                for(i=0;i<SIZE3;i++){
                    if(i<train_length){
                        train_label[i]=(int)buf[i];
                    }
                    else{
                        validation_label[i-train_length]=(int)buf[i];
                    }
                }
            }
...
void loaddata_image(char* address,int length){
    ...
    int index=0;
    fp = fopen(address,"r");
    float p_data[SIZE3*NUMIN];
    if(fp!=NULL&&fread((char*)p_data,1,SIZE3*NUMIN*sizeof(float),fp) >=0){
        puts("success");
        if(length==0){
                for(i=0;i<test_length;i++){
                    for(j=0;j<NUMIN;j++){
                        test_image[i][j]=p_data[index++];
                    }
                }
        }
        else{
            for(i=0;i<SIZE3;i++){
                if(i<train_length){
                    for(j=0;j<NUMIN;j++){
                        train_image[i][j]=p_data[index++];
                    }
                }
                else{
                    for(j=0;j<NUMIN;j++){
                        validation_image[i-train_length][j]=p_data[index++];
                    }
               }
           }
       }
