...
void initWeight(int me, struct Pars *pars, bool type);
void saveFile(string filename, int length, float *data);
...
int main(int argc, char **argv)
{
    ...
    pars->out_size = pars->input_size - pars->filter_size + 2;
    pars->pooling_size = 3;
...
void assignMemory(struct Pars *pars)
{
    ...
    //3.初始化最后的r
    int r_size = pars->input_size*pars->input_size*pars->input_channels*pars->batch_size;
    pars->send_reconstruct = new float[r_size];
    pars->receive_reconstruct = new float[r_size];
    //4.初始化最后的h
    int h_size = pars->out_size*pars->out_size*pars->filter_channels*pars->batch_size;
...
void initInput(int me, struct Pars *pars, int batch_idx, int process_idx)
{
    ...
    for(int i = 0; i < pars->batch_size; i++)
    {
        ...
        {
            long start = batch_idx*pars->batch_size*pars->input_channels*pars->input_size*pars->input_size \
...
void managerNode(int me, struct Pars *pars)
{
    ...
    // dist.append(".bin");
    int length = pars->out_size*pars->out_size*pars->filter_channels*pars->filter_size*pars->filter_size*pars->input_channels;
    saveFile("weight.bin", length, pars->receive_weight);
    length = pars->input_size*pars->input_size*pars->input_channels*pars->batch_size;
    saveFile("reconstruct.bin", length, pars->receive_reconstruct);
    length = pars->out_size*pars->out_size*pars->filter_channels*pars->batch_size;
    saveFile("hidden.bin", length, pars->receive_hidden);
...
{
    assignMemory(pars);
    int weight_length = pars->out_size*pars->out_size*pars->filter_channels*pars->filter_size*pars->filter_size*pars->input_channels;
    ...
    //初始化权重
    initWeight(me, pars, true);
    MPI_Allreduce(pars->send_weight, pars->receive_weight, weight_length, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
    //大概到100个epoch,30个minibatch开始反向增大
    for(int epoch = 0; epoch < 2; epoch++)
    {
        for(int batch_idx = 0; batch_idx < 2400/pars->batch_size; batch_idx++)
        {
            if(me == 0)
            {
                if(batch_idx == 0)
...
void updateW(int me, struct Pars *pars, int batch_idx)
{
    ...
    {       	
        ...
        //更新权重
        int length = pars->filter_size*pars->filter_size*pars->filter_channels*pars->input_channels*pars->block_size*pars->block_size;
        catlas_saxpby(length, 1, block_dw1, 1, 1, block_dw2, 1);
...
{
    int length = pars->filter_size*pars->filter_size*pars->filter_channels*pars->input_channels*pars->block_size*pars->block_size;
    int start = process_idx*pars->out_size/pars->step*length + me*length;
