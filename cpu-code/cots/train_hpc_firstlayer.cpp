...
int main(int argc,char **argv){
    ...
    //const string inputFileName="../data/stl10/unlabeled_X.bin";
    const string inputFileName="../data/stl10/whiten_unlabeled_X.bin";
    const int picture_number=600;
    //test
    const int testpicture_number=600;
    const string testinputFileName="../data/stl10/train_X.bin";
    ...
    //E=E1+ela*E2
    const float ela=0.001;
    //w=w+learning_rate*delta_W+u*delta_W_m
    const float learning_rate=0.8;
    const float alpha_learning_rate=0.01;
    const float u=0.005;
    const string outfilename="first_w_t.dat";
    const string output_filename="first_outcome_t.dat";
    const string R_0_file="first_R0_t.dat";
    const string R_n_file="first_Rn_t.dat";

    //lcn layer
    const int lcn_channel=8;
    const int lcn_row=3;
    const int lcn_col=3;
    ...
    firstlayer.set_output_filename(output_filename);
    ...
    for(int epoch=0;epoch<n_epochs;epoch++){
        ...
        for(int minibatch_index=0;minibatch_index<numberOfminibatch;minibatch_index++){
            ...
            float delta_alpha=0;
            if(rank==1){
                delta_alpha=firstlayer.compute_delta_alpha2();
                delta_alpha+=firstlayer.compute_delta_alpha1();
            }
            MPI_Bcast(&delta_alpha,1,MPI_FLOAT,1,MPI_COMM_WORLD);
            if(rank==0) cout<<"alpha:"<<alpha<<"\tdelta_alpha:"<<alpha_learning_rate*delta_alpha<<endl;
        ...
        //each epoch runing a test set
        firstlayer.set_inputFileName(testinputFileName);
        firstlayer.set_picture_number(testpicture_number);
        int test_numberOfminibatch=testpicture_number/minibatch;
        float R_cost=0;
        for(int i=0;i<test_numberOfminibatch;i++){
            if(rank==0){
                firstlayer.loaddata_Integer(i);//change pixelD
                firstlayer.preprocess();
            }
            float *pixelD=firstlayer.get_pixelD();
            MPI_Bcast(pixelD,size_pixelD,MPI_FLOAT,0,MPI_COMM_WORLD);
            ...
            catlas_saxpby(size_pixelD,-1,pixelD,1,1,rpixelD_gather,1);
            if(rank==0){
                for(int j=0;j<size_pixelD;j++){
                    R_cost+=rpixelD_gather[j]*rpixelD_gather[j];
                }
            }
        }
        if(rank==0){
            R_cost=R_cost/test_numberOfminibatch;
            cout<<"test\tepoch:"<<epoch<<"\tR_cost:"<<R_cost<<endl;
            cout<<"-----------------------------------------------"<<endl;
        }
    }
